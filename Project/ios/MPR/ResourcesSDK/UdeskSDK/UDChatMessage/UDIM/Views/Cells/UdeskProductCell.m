...
- (UIButton *)productSendButton {
    ...
    if (!_productSendButton) {
        ...
        NSString * productSendText;
        if ([UdeskSDKConfig sharedConfig].productSendText) {
          productSendText = [UdeskSDKConfig sharedConfig].productSendText;
        }
        else {
          productSendText = getUDLocalizedString(@"udesk_send_link");
        }
        [_productSendButton setTitle:productSendText forState:UIControlStateNormal];
